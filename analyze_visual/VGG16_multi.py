...
def create_dataset(videos_path):
    ...
    for folder in videos_path:
        ...
        for filename in os.listdir(folder):
            if filename.endswith("_VGG16.npy"):
...
def load_data(X, y, check_train, scaler, pca):
    ...
    if pca is None:
        pca = TimeSeriesPCA(n_components=0.85)
...
def data_preparation(videos_dataset, batch_size):
    ...
    X_val, y_val, val_lengths = load_data(X_val, y_val, False, scaler=scaler,pca=pca)
    val_dataset = LSTMDataset(X_val, y_val, val_lengths)

    X_test, y_test, test_lengths = load_data(X_test, y_test, False, scaler=scaler,pca=pca)
...
if __name__ == "__main__":
    ...
    for folder in videos_path: # for each class-folder
        ...
        # been saved):
        np_feature_files = fnmatch.filter(os.listdir(folder), '*_VGG16.npy')
    ...
    np.save(str(len(videos_path)) + "_VGG16_LSTM_y_test.npy", vals)
    np.save(str(len(videos_path)) + "_VGG16_LSTM_y_pred.npy", preds)
