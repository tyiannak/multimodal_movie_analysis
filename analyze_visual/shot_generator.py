...
def process_video(video_path, process_mode, print_flag, save_results):
    ...
    shot_timestamp = [0,0,0]
    shot_timestamp = np.array(shot_timestamp)
                ...
                if process_mode > 1:
                    ...
                    if count_process > 1:
                        ...
                        if shot_change(gray_diff, mag_mu, f_diff,
                            ...
                            movie_time = capture.get(cv2.CAP_PROP_POS_MSEC)
                            ...
                            hrs,mins,secs = millisec_to_time(movie_time)

                            print(hrs,":",mins,":",secs)

                            shot_timestamp=np.append(shot_timestamp, [hrs,mins,secs])
    ...
    if print_flag:
        ...
        print('Number of shots in video: ',shot_counter)

        shot_timestamp = shot_timestamp.astype(int)

        print('Timestamps of shots in video: \n',shot_timestamp) #print the timestamps of shots

        
        
        
        for index in range(0,len(shot_timestamp),3):

            try:

                print (shot_timestamp[index])
            
                i = shot_timestamp[index]

                if i < shot_timestamp[-1]:
                    
                    ffmpeg_extract_subclip(video_path, i, shot_timestamp[index+1],
                     targetname=str(shot_timestamp[index])+"_"+str(shot_timestamp[index+1])+"_"+
                     str(shot_timestamp[index+2])+"*"+str(shot_timestamp[index+3])+
                    "_"+str(shot_timestamp[index+4])+"_"+str(shot_timestamp[index+5])+".mp4")
